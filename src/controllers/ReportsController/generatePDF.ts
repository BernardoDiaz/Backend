...
import { other_generatePDF } from '../../models/ReportsModel/othergeneratePDF';
...
export const viewTicket_other = async(req:Request,res:Response) =>{
    const {id} = req.params
    try {
        const list = await other_generatePDF.findAll({
            attributes:['pdf'],
...
export const newTicket_other = async (req: Request, res: Response) => {
    ...
    // Guarda el objeto Buffer en la base de datos
    await other_generatePDF.create({
